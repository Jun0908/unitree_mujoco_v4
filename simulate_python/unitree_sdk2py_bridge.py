...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data):
        ...
        self.joystick = None
        self.last_cmd_debug_time = 0.0
        self.local_cmd_path = os.environ.get(
            "UNITREE_MUJOCO_LOCAL_CMD_PATH", "/tmp/unitree_mujoco_lowcmd.json"
        )
        self.local_cmd_mtime = 0.0
    ...
    def LowCmdHandler(self, msg: LowCmd_):
        if self.mj_data != None:
            self._apply_motor_cmd(msg)

    def _apply_motor_cmd(self, msg):
        ...
        if now - self.last_cmd_debug_time >= 1.0 and self.num_motor > 16:
            joint_id = 16
            print(
                "sim_debug "
                f"joint={joint_id} "
                f"target_q={msg.motor_cmd[joint_id].q:+.3f} "
                f"kp={msg.motor_cmd[joint_id].kp:+.1f} "
                f"sensor_q={self.mj_data.sensordata[joint_id]:+.3f} "
                f"ctrl={self.mj_data.ctrl[joint_id]:+.3f}"
            )
            self.last_cmd_debug_time = now

    def ApplyLocalLowCmdIfAvailable(self):
        try:
            mtime = os.path.getmtime(self.local_cmd_path)
        except OSError:
            return

        if mtime <= self.local_cmd_mtime:
            return

        try:
            with open(self.local_cmd_path, "r", encoding="utf-8") as f:
                payload = json.load(f)
        except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError):
            return

        motor_cmd = payload.get("motor_cmd", [])
        if len(motor_cmd) < self.num_motor:
            return
        ...
        for i in range(self.num_motor):
            item = motor_cmd[i]
            motor_view = _MotorCmdView()
            motor_view.mode = item["mode"]
            motor_view.q = item["q"]
            motor_view.dq = item["dq"]
            motor_view.kp = item["kp"]
            motor_view.kd = item["kd"]
            motor_view.tau = item["tau"]
            cmd_view.motor_cmd.append(motor_view)

        self._apply_motor_cmd(cmd_view)
        self._apply_local_joint_targets(cmd_view)
        self.local_cmd_mtime = mtime

    def _apply_local_joint_targets(self, msg):
        # Local fallback for visualization: write commanded joint positions
        # directly into MuJoCo state so motion is visible even when DDS/torque
        # behavior is unreliable in this environment.
        for actuator_id in range(self.num_motor):
            joint_id = self.mj_model.actuator_trnid[actuator_id][0]
            if joint_id < 0:
                continue
            qpos_adr = self.mj_model.jnt_qposadr[joint_id]
            qvel_adr = self.mj_model.jnt_dofadr[joint_id]
            self.mj_data.qpos[qpos_adr] = msg.motor_cmd[actuator_id].q
            self.mj_data.qvel[qvel_adr] = msg.motor_cmd[actuator_id].dq
